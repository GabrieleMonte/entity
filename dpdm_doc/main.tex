...
LZ high amplitude & 1.0 & $2.7\times10^3$ & 1 A100, 2:39:32 & complete, $\wpt = 2\times10^4$ \\
...
\paragraph{Measured result at $v_q^D/\vth = 1.0$, and a discrepancy.} The run
completed the full sweep. Extracted from the stats series:
\[
  \max(\epsilon_E)/\epsilon_{\rm th} = 190.4 \ \text{at}\ \wpt = 3689, \qquad
  \Delta KE_e/\epsilon_{\rm th} = 996 \ \text{at the crossing},\ \ 1002 \ \text{at}\ \wpt = 2\times10^4 .
\]
The qualitative claim reproduces: the transfer peaks at $\wpt \approx 3700$, well
\emph{before} the crossing at $\wpt = 10^4$, and is strongly suppressed relative to a
resonant drive at the same amplitude --- the resonant $\tfrac18 A_0^2(\wpt)^2$ law
would reach ${\sim}6\times10^6\,\epsilon_{\rm th}$ by $\wpt = 5000$ against our
${\sim}10^3$. (The paper's own linear line for this figure is Eq.~(LZ), not the
resonant law, so that comparison is a sanity check rather than a reproduction.)

\textbf{The absolute heating does not reproduce.} The paper states of this case ---
explicitly the large-amplitude one, ``because the initial $v_q/\vth$ is chosen so
large'' --- that ``the kinetic energy of the electrons is not even changed by a factor
of 2''. We measure a factor ${\sim}10^3$. The two inputs that would explain this away
both match the source: the amplitude ($v_q^D/\vth = 1$, the upper panel of the paper's
Landau--Zener figure) and the sweep ($\omega(t) = 0.8\omega_p + 0.1\omega_p t/5000$,
stated verbatim in the paper's appendix). This is unresolved and is the most important
open item; see \S\ref{sec:status}.
...
gate at Landau--Zener duration, the full performance characterisation, and the
Landau--Zener run at $v_q^D/\vth = 1.0$ (2:39:32 on one A100, full sweep to
$\wpt = 2\times10^4$). It took 2:39:32 against an estimate of ${\sim}2$\,h --- it would
not have fitted a 2\,h queue. That estimate predated the post-saturation slowdown of
\S\ref{sec:slowdown}, which applies to the swept family too.

All six production runs have been reduced to \code{.npz} by
\code{pgens/dpdm/paper/extract\_paper.py} (58 minutes on one \code{development} node;
1.9\,GB of \code{.bp} to 22\,MB). That extraction independently reproduces the
resonance numbers quoted in \S\ref{sec:resonance} --- fast: peak
$\epsilon_E/\epsilon_{\rm th} = 43.55$ at $\wpt = 553$, $\Delta KE_e = 69.63$; slow:
$2.13$ at $\wpt = 3694$, $2.43$.

\paragraph{Blocked on interpretation.} The Landau--Zener case at $v_q^D/\vth = 1.0$
disagrees with the paper's stated electron heating by ${\sim}500\times$
(\S\ref{sec:lz}). The $0.1$ case is written but should stay gated until this is
understood: running it would cost ${\sim}81$ GPU-hours to produce a second number
against the same unresolved normalisation.

\paragraph{Open.}
\begin{itemize}[nosep]
  \item \textbf{The Landau--Zener heating discrepancy (\S\ref{sec:lz}), now the
        blocking item.} Our $v_q^D/\vth = 1.0$ run heats the electrons by a factor
        ${\sim}10^3$ where the paper reports less than $2$. Amplitude and sweep rate
        both match the source. The next step is to digitise the published
        Landau--Zener figure: it is the only way to tell whether the disagreement is in
        the normalisation of the quoted statement or in the physics the drive produces
        at $v_q \approx \vth$. Worth checking specifically whether $A_0$ should be the
        field amplitude or the quiver velocity $v_q = eE/(m\omega)$, which differ by
        $1/\omega = 1.25$ at $\omega = 0.8\omega_p$ --- too small to explain
        ${\sim}500\times$ on its own, but it is the kind of definitional slip that
        compounds.
  \item \textbf{No comparison against the published figures} more generally. Every
        other number here matches the paper's \emph{text}, but no figure PDF in the
        source package has been digitised. Peak $\epsilon_E/\epsilon_{\rm th}$ and its
        timing are the sharpest available discriminators.
